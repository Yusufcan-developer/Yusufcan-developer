...
import { Col, Row, Button, Pagination, Collapse, Spin, Badge, notification, Typography, Tooltip, message, Input } from "antd";
...
const SearchComponent = () => {
  ...
  async function onAddProductCart(product, orderPartialAddTobox = false, isPartial = false, selectedQuantity = 0) {
    if ((product.hasDependentOrRelatedProducts === true) || (product.canBeSoldPartially === true)) {
      await getProductDetail(product.itemCode);
